...
let practicalsHidden = true;

function togglePracticals() {
    const practicals = document.querySelectorAll('.event.practical');
    const btn        = document.getElementById('togglePracticals');
    practicalsHidden = !practicalsHidden;
    practicals.forEach(e => e.classList.toggle('hidden-practical', practicalsHidden));
    btn.textContent = practicalsHidden ? 'Show Practicals' : 'Hide Practicals';
    btn.classList.toggle('active', practicalsHidden);
}
...
if (window.innerWidth <= 768) {
    ...
    document.querySelectorAll('.event.practical').forEach(e => e.classList.add('hidden-practical'));
}
